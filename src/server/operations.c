...
char kvs_subscribe(const char *key, int fd){
  pthread_rwlock_rdlock(&kvs_table->tablelock);
  char result = key_subscribe(kvs_table, key, fd);
  pthread_rwlock_unlock(&kvs_table->tablelock);
  return result ;
}

char kvs_unsubscribe(const char *key, int fd){
  pthread_rwlock_rdlock(&kvs_table->tablelock);
  char result = key_unsubscribe(kvs_table, key, fd);
  pthread_rwlock_unlock(&kvs_table->tablelock);
  return result ;
}

void kvs_killAllClients(){
  pthread_rwlock_rdlock(&kvs_table->tablelock);
  kill_all_clients(kvs_table);
  pthread_rwlock_unlock(&kvs_table->tablelock);
}
